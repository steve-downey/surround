...
The overdone project infrastructure in the top-level project is to support GitHub CI and various bits of code quality. The CI is mostly driven by shell scripts in the .ci directory, which are borrowed from the and were written by [Przemysław Kryger](https://github.com/pkryger) who despaired of us breaking our shared emacs configuration. The [pre-commit framework](https://github.com/pre-commit/pre-commit) infrastructure is my fault, but helps me keep YAML less broken, and reduced the number of spelling mistakes and formatting errors. Running it automatically keeps me honest.
...
This project also has overdone infrastructure, largely borrowed from the [Beman Project](https://github.com/bemanproject), reusing the infra from and .
